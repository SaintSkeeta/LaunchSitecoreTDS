...
namespace HedgehogDevelopment.CodeGeneration.Extensions
{
    public static class StringExtensions
    {
        ...
        public static string GetNamespaceFromPath(this string path)
        {

            string pathReplaced = path.Replace('/', '.').Replace('\\', '.');
            int indexOfLastDot = pathReplaced.LastIndexOf('.');
            
            if (indexOfLastDot > 0)
            {
                pathReplaced = pathReplaced.Substring(0, indexOfLastDot);
            }
        ...
        /// <summary>
        /// Takes a single string, and splits the sentences up by the newline character.
        /// </summary>
        /// <param name="originalString"></param>
        /// <returns></returns>
        public static IEnumerable<string> AsSentences(this string originalString)
        {
            var sentences = new List<string>();
            var splitLines = originalString.Split(new string[] { "\r\n", "\n" }, StringSplitOptions.None);
            sentences.AddRange(splitLines);
