namespace LaunchSitecore.Data.Fields
{
  ...
  {
    public LayoutField(SC.Data.Fields.Field innerField) : base(innerField)
    {
    }

    public LayoutField(SC.Data.Items.Item item) : base(item)
    {
    }
    ...
    public override void ValidateLinks(
      SC.Links.LinksValidationResult result)
    {
      ...
      ArrayList devices = SC.Layouts.LayoutDefinition.Parse(
        xml).Devices;
      ...
      foreach (SC.Layouts.DeviceDefinition device in devices)
      {
        foreach (SC.Layouts.RenderingDefinition rendering 
          in device.Renderings)
        {
          if (String.IsNullOrEmpty(rendering.Datasource)
          ...
          SC.Data.Items.Item dataSource = this.InnerField.Database.GetItem(
